...
# NAVIGATE TO WHERE ZAP IS LOCATED
# START ZAP IN DAEMON MODE AND DISABLE THE API KEY


#subprocess.run('./zap.sh -daemon -config api.disablekey=true', shell=True, cwd='/opt/ZAP_2.12.0/')


# MANDATORY. Define the API key generated by ZAP and used to verify actions.
apiKey = '1nuqco947hkf4rlcp1m75fp4bg'
...
# The following example excludes every single URL except http://localhost:8081 ['^(?:(?!http:\/\/localhost:8081).*).$']
globalExcludeUrl = []
...
contextIncludeURL = ['http://testphp.vulnweb.com/']
...
# name/value a pair of authParams are expected to be "x-www-form-urlencoded"
...
ascanIds = [40018, 40019, 40020, 40021, 40022, 40024, 90018, 40033,
...
def runZap(target, useScanPolicy, ascanIds, attackStrength):
    ...
    # define the commands to run in the new terminal window

    print("OPENING ZAP IN NEW TERMINAL")

    commands = ['cd /opt/ZAP_2.12.0', './zap.sh -daemon -config api.disablekey=true']

    # join the commands into a single string
    command_string = '; '.join(commands)

    # open a new terminal window and run the commands
    subprocess.run(['gnome-terminal', '--', 'bash', '-c', command_string])
    ...
    # Path and filename of the output PDF file
    output_path = "/home/kali/" + reportname + ".pdf"
    ...
    print("[*] Saving ", reportname, " at location: ", output_path)


    # WRITE A CODE TO
    # OPEN DEFAULT BROWSER AND NAVIGATE TO : http://localhost:8080/OTHER/core/other/htmlreport
    # THEN PRINT THE PAGE AND SAVE TO LOCAL DIRECTORY AS PDF (WRITE A CODE FOR THIS AS WELL)
    #
